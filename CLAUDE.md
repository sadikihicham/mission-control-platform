...
**Known gap.** The V1 realtime channel (`/agent-control/ws`, Redis channel `ac:events`) is documented in the contract and the outbox does write rows (`apps/api/agent_control/operations/outbox.py`, table `mc_outbox_events`), but nothing consumes/relays them yet — events sit `pending` forever. Only the legacy V0 `/ws` is wired end-to-end.
...
- Agent Control test suite: 14 `apps/api/tests/test_agent_control_*.py` files, including cross-tenant isolation tests that assert **404, never 403** (see the Architecture subsection above).
